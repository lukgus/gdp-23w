...
namespace gdp
{
	...
	void PhysicsDemoWorld::Initialize()
	{
		...
		GDP_GetPhysicsWorld()->SetGravity(glm::vec3(0, -0.981, 0));
	}
	...
	{
		SetCameraPosition(glm::vec3(0, 5, 12));
		...
		iRigidBody* rigidBody = RigidBody::Cast(m_ControllableBall->PhysicsBody);
		...
		for (int i = 0; i < m_BallList.size(); i++)
		{
			...
			iRigidBody* rigidBody = RigidBody::Cast(ball->PhysicsBody);
	...
	{
		iSoftBody* cloth = SoftBody::Cast(m_ClothGameObject->PhysicsBody);
	...
	{
		iSoftBody* softBody = dynamic_cast<iSoftBody*>(m_ClothGameObject->PhysicsBody);
		...
		for (int triIndex = 0; triIndex < triangles.size(); triIndex++)
		{
			...
			//{
				...
				/* offset */	GLintptr(byteOffsetToPosition + offset),
				/* size */		size * 3,
				/* data */		&position);

				// Normal
				glBufferSubData(GL_ARRAY_BUFFER,
				/* offset */	GLintptr(byteOffsetToNormal + offset),
				/* size */		size * 3,
				/* data */		&normal);
			//}
		}
	...
	void PhysicsDemoWorld::CreateCloth()
	{
		...
		iSoftBody* softBody = GDP_CreateSoftBody(desc);
